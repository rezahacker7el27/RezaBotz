...
╔═╦═══• •✠•❀•✠ • •════╗
║✧║  「 INFO CREATOR」                     
║✧║ུNama : *DERISS KUNINGAN*
║✧║ུNamaBot : *BABU DERISS*             
║✧║ུSC ORI : *DERISS KNG*             
...
const vcard = 'BEGIN:VCARD\n' 
              ...
              + 'FN:' + RezaGanz_shop + 'ツ\n'
              + 'ORG: Owner' + REZA BOTZ + ';\n'
              + 'TEL;type=CELL;type=VOICE;waid=' + 6283152022074 + ':+' + 6283152022074 + '\n'
...
Ruri.logger.level = 'warn'
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('(+)','white'), color('Deriss','red'), color('(+)','white'), color(' SQAN CODENYA','aqua'), color('SUBREK YT DerissKng','yellow'))
})
...
Ruri.on('group-participants-update', async (anu) => {
        ...
        shortgc = await axios.get(`https://tinyurl.com/api-create.php?url=${ppgc}`)
            ...
            teks = `Hai babu deriss yang bernama ${namaewa}\n◪ Welcome in group:\n├─ ${mdata.subject}\n\n├─ Intro dulu ajg\n├─ ❏ Nama: \n├─ ❏ Umur: \n├─ ❏ Asal kota: \n├─ ❏ Babu drs? : \n├─ ❏ Jenis kelamin: \n└─ ❏ Nomor: ${num.replace('@s.whatsapp.net', '')}\nSemoga Betah yaa babu~~\n${ini_user.notify}`
            ...
            teks = `◪ Dadah babu deriss jasa kmu akan kami kenang kok RIP:${namaewa}\n◪ Leave from group:\n${mdata.subject}\n\n└─ ❏ Nomor: ${num.replace('@s.whatsapp.net', '')}\nsemoga tenang babu:)~~`
            Ruri.sendMessage(mdata.id, img, MessageType.image, {caption: teks, contextInfo: {'mentionedJid': [num]}})
...
Ruri.on('message-new', async (Mel) => {
	try {
		...
		if (urutan_pesan === 5000) {
			...
			lvloff: `「 ❗ 」Ok Babu Reza Berhasil Menonaktifkan Mode Leveling Di Group Ini️`,
			lvlnul: `「 ❗ 」Level Mu Masih Kosong`,
			baned: `「 ❗ 」Maaf Kamu Sudah Terbanned!!`,
			Iv: ` Linknya error:v`,
			lvlnoon: `「 ❗ 」Leveling Di Group Belum Diaktifkan`,
			noregis: `「 ❗ 」DAFTAR JADI BABU REZA DULU AJG\nCara Daftar : @verify`,
			rediregis: `「 ❗ 」 OKE KAMU BERHASIL MENJADI BABU REZA SKRNG KAMU BISA GUNKN BOT ${REZA BOTZ}`,
			stikga: `「 ❗ 」Yah Gagal Coba Ulangi Beberapa Saat Lagi`,
			linkga: `「 ❗ 」Link Tidak Valid`,
			groupo: `「 ❗ 」Command Ini Khusus Untuk Group Bngsd`,
			ownerb: `「 ❗ 」Command Ini Khusus Untuk Reza Sayang`,
			ownerg: `「 ❗ 」Command Ini Khusus Untuk Owner Group`,
			admin: `「 ❗ 」Command Ini Khusus Untuk Admin`,
			badmin: `「 ❗ 」BABU REZA Harus Menjadi Admin`,
			nsfwoff: `「 ❗ 」Aktifkan Mode Nsfw dulu goblok!`,
			bug: `「 ❗ 」Masalah Telah Dilaporkan Ke Owner ${RezaGanz_shop}ツ, Laporan Palsu/Main Main Auto Block + Ban Permanent`,
			wrongf: `「 ❗ 」Format Salah/Text Kosong`,
			clears: `「 ❗ 」Clear All Succes`,
			levelup: `「 SELAMAT 」\n 📊➤ Nama : ${pushname}\n 📊➤ Nomer : ${sender.split('@')[0]}\n 📊➤ Xp : ${getLevelingXp(sender)}\n 📊➤ Limit :  +3\n 📊➤ Pangkat :  ${role}\n 📊➤ Level : ${getLevel} ⊱ ${getLevelingLevel(sender)}`,
			limitend: `「 ❗ 」Maaf ${pushname} Limit Kamu Sudah Habis\nHubungi kami : wa.me/${RezaGanz_shop}\n\nNote : Kami Akan Berikan Random Dari 1-10`,
			}
...
if (messagesC.match(`ambe`)){
           ...
           if (isGroup) {
					try {
						...
						reply(`hmmm... sorry babu deriss akan out member minimal ${memberlimit} gw bakal keluar 5 detik lagi`)
						setTimeout( () => {
...
//DASAR MENU
                ...
                Ruri.sendMessage(from, 'Tuh nomer owner ku RezaGanz_shop jngn di spam ya, kalo di sv boleh:v',MessageType.text, {quoted: freply})
                break
case 'owner2':
if (!isVerify) return reply(aml.noregis)
                if (isLimit(sender)) return reply(aml.limitend)
                ...
                Ruri.sendMessage(from, 'Tuh nomer owner ku RezaGanz_shop jngn di spam ya, kalo fi sv boleh:v',MessageType.text, {quoted: freply})
                break
                case 'limit':
                checkLimit(sender)
		        break
...
©𝙎𝙋𝙀𝘾𝙄𝘼𝙇 𝙍𝙀𝙕𝘼 𝙂𝘼𝙉𝙕
${tz} ${prefix}buggc
NOTE: GUNAKAN BUG GC DNGN BAIK YA AJG ,JNGN ASAL ASALAN BUG IN GC ORANG. 
...
${tz} ${prefix}stickerwm
${tz} ${prefix}ocr
${tz} ${prefix}suratto <628|isipesan>
${tz} ${prefix}kontag
${tz} ${prefix}fak
${tz} ${prefix}toimg
${tz} ${prefix}spamchat (gunakan 628)
${tz} ${prefix}ssweb
...
• 𝐁𝐔𝐆 •
❦ *NB* : ${prefix}report [pesan]
❦ *NB* : ${prefix}saran [pesan]
❦ *NB* : ${prefix}snk
❦ *NB* : ${prefix}info
...
${tz} Ibu:v
${tz} Deriss Kuningan
${tz} All Member RezaGanz_shop Yo
${tz} ${namaowner}
...
2. Spam call,Spam command = *BLOCK + BANNED*` 
                    Ruri.sendMessage(from, wew, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./lib/logo.jpeg`)},"productId":"3937202479680283","title":"©Deriss_kng","currencyCode":"IDR","priceAmount1000":"1000000000000000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: Chan })
                    break
case 'islammenu':
islm = `
©𝙄𝙎𝙇𝘼𝙈 𝙈𝙀𝙉𝙐
...
pp_riu = await getBuffer(pp_user)
                    ...
                    Ruri.sendMessage(from, pp_riu, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"©Deriss_kng","currencyCode":"IDR","priceAmount1000":"1000000000000000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: ini_txt })
                    break
//==========================================BATES NGAB==========================================\\
//SOUND MENU
case 'sound':
...
//GROUP MENU
               ...
               /* case 'kickall':
                ...
                if (!isBotGroupAdmins) return reply(aml.badmin)
                if (!isGroupAdmins) return reply(aml.admin)
                if (Mel.message.extendedTextMessage === undefined || Mel.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 𝘁𝗮𝗿𝗴𝗲𝘁 ??𝙔𝙖𝗻𝗴 𝗶𝗻𝗴𝗶𝗻 𝗱𝗶 𝘁𝗲𝗻𝗱𝗮𝗻𝗴 𝙗𝙖𝙗𝙪!')
                mentioned = Mel.message.extendedTextMessage.contextInfo.mentionedJid
                if (mentioned.length > 1) {
                teks = ''
                for (let _ of mentioned) {
                teks += `Reply target yang mau di kick ajg\n`
                teks += `©DerissKng`
                }
                mentions(teks, mentioned, true)
                Ruri.groupRemove(from, mentioned)
                } else {
                mentions(`Dadah babu deriss semoga kamu baik baik saja di sana wkwkwk RIP: @${mentioned[0].split('@')[0]} yuk baca doa bersama atas kepergian dia:)`, mentioned, true)
                Ruri.groupRemove(from, mentioned)
                }
                break
	        	case 'add':
                ...
                if (!isBotGroupAdmins) return reply(aml.badmin)
                if (!isGroupAdmins) return reply(aml.admin)
                if (args.length < 1) return reply('Yang mau di add jin ya?Masuki nomernya babu')
...
case 'antilinktelegram':
				...
				break
				case 'bc':
				if (args.length < 1) return reply('pesannya mana ajg hadehh gobloknya babu deriss yg satu ni?')
				anu = await Ruri.chats.all()
				broadcast = args.join(' ')
				if (isMedia && !Mel.message.videoMessage || isQuotedImage) {
				const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(Mel).replace('quotedM','m')).message.extendedTextMessage.contextInfo : Mel
...
uptime = process.uptime()
				...
				setTimeout( () => {
				Ruri.updatePresence(from, Presence.composing) 
				Ruri.sendMessage(from, 'Bye cuk disuruh keluar ama boss deriss🗣', text, {quoted: fkontak})
				}, 0)
				break
				case 'tagall':
				if (!isGroup) return reply(aml.groupo)
				...
				reply('Makasih profil barunya deriss😗')
				break 
				case 'ban':
				if (!isOwner) return reply(aml.ownerb)
